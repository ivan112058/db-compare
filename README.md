...
## 🛡️ Safe SQL Execution (Idempotent SQL)

The Upgrade and Rollback scripts generated by DB Compare are designed to be **idempotent**. This means that before executing any changes, the script queries the `INFORMATION_SCHEMA` system tables to check the current state of the database. Therefore, even if the script is executed repeatedly (e.g., in a CI/CD pipeline), it will not fail due to objects already existing or missing, ensuring a safe deployment process.

| Scenario | Execution Logic | Example SQL (Simplified) |
| :--- | :--- | :--- |
| **Create Table** | Check if table exists, create only if missing | `CREATE TABLE IF NOT EXISTS system_log ...` |
| **Add Column** | Check if column exists, add only if **missing** | `SET @preparedStatement = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE ...)>0, 'SELECT 1', 'ALTER TABLE ... ADD COLUMN ...')); PREPARE stmt FROM @preparedStatement; EXECUTE stmt;` |
| **Drop Column** | Check if column exists, drop only if **exists** | `SET @preparedStatement = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE ...)>0, 'ALTER TABLE ... DROP COLUMN ...', 'SELECT 1')); PREPARE stmt FROM @preparedStatement; EXECUTE stmt;` |
| **Add Index** | Check if index exists, add only if **missing** | `SET @preparedStatement = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE ...)>0, 'SELECT 1', 'ALTER TABLE ... ADD INDEX ...')); ...` |
| **Drop Index** | Check if index exists, drop only if **exists** | `SET @preparedStatement = (SELECT IF((SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS WHERE ...)>0, 'ALTER TABLE ... DROP INDEX ...', 'SELECT 1')); ...` |
...
├── dbc/                     # Database compare service (Kotlin + Ktor)
